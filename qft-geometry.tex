...
\item First we note that the manifold $X=S^1$ can only have $\Omega^0(X)$ and $\Omega^1(X)$ (because higher forms vanish). Next, we put a flat metric $g_{00}=1$ on the circle $S^1$.  We consider $\alpha\in\Omega^1(X)$ and $\beta\in\Omega^0(X)$ then in the chart $\theta:S^1\to\mathbb{R}$, $\alpha = f(\theta)d\theta$ and $\beta=h(\theta)$.  Now $d^*$ can be evaluated as follows
  \begin{equation}
    ...
    \end{split}
  \end{equation}
Now the Laplacian $\Delta=dd^*+d^*d$ acting on $\alpha\in\Omega^0(S^1)$ is essentially $d^*d\alpha$ (because $d^*$ annhiliates $\alpha$).  Therefore in the chart $\theta$, $\Delta=\partial^2_\theta$.  Now $\mathcal{H}_0(X)=\{\omega\in\Omega^0(X)|\partial^2_\theta\omega(\theta)=0\}$.  The solution $\omega(\theta)=a\theta+b$ resulting in $b_0(S^1)=2$ (?).
