...
\begin{equation}
  b_k(X) = \dim_\R \cH_k(X).
\end{equation}
There is a very remarkable fact about these integers:
...
\begin{solution}
  The measure $d\mu=d\psi^1d\psi^2d\psi^3d\psi^4\in\wedge^4(\Pi\R^{0|4})$ leads to the following integral
  \begin{equation}
  \begin{split}
    Z&=\int_{\mathcal{C}}d\psi^1d\psi^2d\psi^3d\psi^4 e^{-(m\psi^1\psi^2+m\psi^3\psi^4+\lambda\psi^1\psi^2\psi^3\psi^4)}\\
     &=\int_{\mathcal{C}}d\psi^1d\psi^2d\psi^3d\psi^4 (1-(m\psi^1\psi^2+m\psi^3\psi^4+\lambda\psi^1\psi^2\psi^3\psi^4)+\frac{1}{2}(m\psi^1\psi^2+m\psi^3\psi^4+\lambda\psi^1\psi^2\psi^3\psi^4)^2)
  \end{split}
  \end{equation}
Now $\int_{\cC}d\psi^1d\psi^2d\psi^3d\psi^4\psi^1\psi^2=\alpha(a1,a2,1,1)=0$ where $\alpha\in\wedge^4(\Pi\R^{0|4})$.  Thus the only surviving terms are $m^2\psi^2\psi^2\psi^3\psi^4$ and $-\lambda\psi^2\psi^2\psi^3\psi^4$.  Thus $Z=m^2-\lambda$
\end{solution}
...
\begin{solution}
  The super-vector space $V$ has a space $Vec(V)$ of graded derivations on $C^\infty(V)$ given by equation \ref{gradedderiv}.  
\end{solution}
...
\begin{defn}[Symplectic form]
  Consider the the $(M,\omega)$ symplectic manifold with the nondegenerate 2-form $\omega$ as follows
  \begin{equation}
    \omega:TM\to T^*M
  \end{equation}
with the inverse
...
<<<<<<< HEAD
Now the one-forms on the symplectic manifold can be identified with vector fields.  Consider a differentiable map $H:M\to\R$ known as the Hamiltonian.  The corresponding one-form is $dH$. Therefore we can associate a vector field $X_H$ with the Hamiltonian in the following way 
\begin{equation}
  X_H=\Omega(dH,\cdot)
\end{equation}
...
\begin{solution}
  The partition function is essentially
  \begin{equation}
  \begin{split}
    Z&=\int_{\cC=\Pi(TM)}d\vec{x}d\vec{\psi}e^{i\alpha (H+\omega_{ij}\psi^i\psi^j)}\\
    &=\int_{\cC}d\vec{x}
  \end{split}
  \end{equation}
\end{solution}
...
\begin{solution}
  ...
  \begin{equation}
   Q \Psi = g(Y,Y) + \de(gY) = Y^i Y_i - \psi^i \psi^k \partial_{x^k} Y_i 
  \end{equation}
Now $Q^2\Psi = Q(Y^i Y_i - \psi^i \psi^k \partial_{x^k} Y_i)$ and in the coordinates we have
\begin{equation}
  \begin{split}
    Q(Y^i Y_i - \psi^i \psi^k \partial_{x^k} Y_i)&= (\psi^i\partial_{x^i}Y^j\partial_{\psi^j}+Y^i\partial_{x^i})(Y^lY_l-\psi^l\psi^k\partial_{x^k}Y_l)\\
&= -\psi^i\partial_{x^i}Y^j\psi^k\partial_{x^k}Y_j+\psi^i\partial_{x^i}Y^j\psi^l\partial_{x^j}Y_l+Y^i\partial_{x^i}(Y^lY_l)-Y^i\psi^l\psi^k\partial_{x^i}\partial_{x^k}Y_l
  \end{split}
\end{equation}
\end{solution}
